...
def rotate_log_if_needed(library_path: str) -> None:
    ...
    # Rotate existing backups: .4.gz -> .5.gz, .3.gz -> .4.gz, etc.
    for i in range(MAX_BACKUP_COUNT - 1, 0, -1):
        old_backup = f"{library_path}.{i}.gz"
        new_backup = f"{library_path}.{i + 1}.gz"
        if os.path.exists(old_backup):
            if i + 1 > MAX_BACKUP_COUNT:
                os.remove(old_backup)
            else:
                shutil.move(old_backup, new_backup)

    # Compress current log to .1.gz
    backup_path = f"{library_path}.1.gz"
    with open(library_path, "rb") as f_in:
        with gzip.open(backup_path, "wb") as f_out:
            shutil.copyfileobj(f_in, f_out)

    # Truncate the original file and add header
    with open(library_path, "w", encoding="utf-8") as f:
        ...
        f.write(f"> Log rotated at {datetime.datetime.now(datetime.timezone.utc).isoformat(timespec='seconds')}\n\n")
        f.write("---\n\n")
...
def main() -> None:
    ...
    rotate_log_if_needed(library_path)
